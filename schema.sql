-- ============================================================
--  AnimeHunt — PostgreSQL Schema v4
--  Сезоны = самостоятельные записи anime, связанные через anime_series.
--  Добавлены: reviews (рецензии к аниме)
-- ============================================================
...
-- ── АНИМЕ ────────────────────────────────────────────────────
-- Каждый сезон — отдельная запись.
-- season_num: NULL если standalone, 1/2/3... если сезон серии.
CREATE TABLE anime (
  id           SERIAL PRIMARY KEY,
  ...
  season_num   SMALLINT,          -- номер сезона внутри серии (NULL = не сезон)
  created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at   TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- ── СЕРИИ (группа связанных сезонов) ─────────────────────────
-- Одна серия объединяет несколько аниме-записей (сезонов).
CREATE TABLE anime_series (
  id          SERIAL PRIMARY KEY,
  title       VARCHAR(255) NOT NULL,     -- «Атака Титанов»
  description TEXT
);

-- M2M: аниме ↔ серия (каждая запись — один сезон серии)
CREATE TABLE anime_series_entries (
  ...
  sort_order INT NOT NULL DEFAULT 0,    -- порядок в серии
...
-- ── РЕЦЕНЗИИ ─────────────────────────────────────────────────
-- Одна рецензия на пользователя на аниме (с оценкой).
...
-- Атака Титанов: 4 сезона как отдельные аниме (id 1-4)
INSERT INTO anime (title,title_en,title_jp,synopsis,status,type,episodes,duration_min,aired_from,aired_to,studio_id,poster_url,is_new,season_num) VALUES
  ('Атака Титанов','Attack on Titan','進撃の巨人',
   ...
   'completed','tv',25,24,'2013-04-07','2013-09-28',6,
   'https://cdn.myanimelist.net/images/anime/10/47347.jpg',FALSE,1),

  ('Атака Титанов 2','Attack on Titan Season 2','進撃の巨人 Season 2',
   'Разведывательный корпус узнаёт тайну происхождения титанов внутри стен. Не все враги снаружи.',
   'completed','tv',12,24,'2017-04-01','2017-06-17',6,
   'https://cdn.myanimelist.net/images/anime/4/84177.jpg',FALSE,2),

  ('Атака Титанов 3','Attack on Titan Season 3','進撃の巨人 Season 3',
   'Политические интриги внутри стен и раскрытие тайны подвала Эрена меняют всё.',
   'completed','tv',22,24,'2018-07-23','2019-07-01',6,
   'https://cdn.myanimelist.net/images/anime/1714/93408.jpg',FALSE,3),
  ...
  ('Атака Титанов: Финальный сезон','Attack on Titan: Final Season','進撃の巨人 The Final Season',
   ...
   'completed','tv',28,24,'2020-12-07','2023-11-05',1,
   'https://cdn.myanimelist.net/images/anime/1000/110531.jpg',FALSE,4),

  -- Магическая битва: 2 сезона (id 5-6)
  ('Магическая битва','Jujutsu Kaisen','呪術廻戦',
   'Юджи Итадори поглощает палец Рёмэна Сукуны и становится сосудом проклятого духа. Начинается путь заклинателя.',
   'completed','tv',24,23,'2020-10-03','2021-03-27',1,
   'https://cdn.myanimelist.net/images/anime/1171/109222.jpg',FALSE,1),
  ...
  ('Магическая битва 2','Jujutsu Kaisen Season 2','呪術廻戦 2期',
   ...
   'https://cdn.myanimelist.net/images/anime/1792/138022.jpg',TRUE,2),

  -- Одиночные тайтлы (id 7-12)
  ('Наруто','Naruto','ナルト',
   'Наруто Узумаки — молодой ниндзя, в котором запечатан демонический лис. Он стремится стать Хокагэ.',
   'completed','tv',220,23,'2002-10-03','2007-02-08',2,
   'https://cdn.myanimelist.net/images/anime/13/17405.jpg',FALSE,NULL),
  ...
  ('Тетрадь смерти','Death Note','デスノート',
   ...
   'https://cdn.myanimelist.net/images/anime/9/9453.jpg',FALSE,NULL),
  ...
  ('Фриерен: За гранью путешествия','Frieren: Beyond Journey''s End','葬送のフリーレン',
   ...
   'https://cdn.myanimelist.net/images/anime/1015/138006.jpg',TRUE,NULL),
  ...
  ('Дандадан','Dandadan','ダンダダン',
   ...
   'https://cdn.myanimelist.net/images/anime/1890/141166.jpg',TRUE,NULL),
  ...
  ('Поднятие уровня в одиночку','Solo Leveling','俺だけレベルアップな件',
   ...
   'https://cdn.myanimelist.net/images/anime/1823/135083.jpg',TRUE,NULL),
  ...
  ('Клинок, рассекающий демонов: Замок Бесконечности','Demon Slayer: Infinity Castle','鬼滅の刃 無限城編',
   ...
   'https://cdn.myanimelist.net/images/anime/1286/99889.jpg',TRUE,NULL);
...
-- Персонажи
INSERT INTO characters (anime_id,name,name_jp,role,description,image_url,age,gender,abilities) VALUES
  (1,'Эрен Йегер','エレン・イェーガー','main','Главный герой. Одержим желанием обрести свободу. Его путь от солдата до разрушителя мира — центр всей саги.','https://cdn.myanimelist.net/images/characters/10/216895.jpg','19','М','Атакующий Титан, Основополагающий Титан, Координата'),
  (1,'Микаса Аккерман','ミカサ・アッカーマン','main','Лучший солдат своего поколения. Приёмная сестра Эрена с уникальными боевыми способностями клана Аккерман.','https://cdn.myanimelist.net/images/characters/9/215563.jpg','19','Ж','Способности клана Аккерман'),
  (1,'Армин Арлерт','アルミン・アルレルト','main','Гениальный тактик. Лучший друг Эрена, чей интеллект не раз спасал человечество.','','19','М','Колоссальный Титан, стратегическое мышление'),
  (8,'Лайт Ягами','夜神月','main','Блестящий студент, нашедший Тетрадь смерти. Стал богом нового мира.','https://cdn.myanimelist.net/images/characters/3/253266.jpg','17','М','Тетрадь смерти, высокий интеллект'),
  (8,'Л','エル','main','Величайший детектив мира.','https://cdn.myanimelist.net/images/characters/5/45867.jpg','25','М','Дедуктивное мышление'),
  (5,'Юджи Итадори','虎杖悠仁','main','Добродушный юноша со сверхсилой, ставший сосудом Сукуны.','','15','М','Сверхсила, Чёрная вспышка'),
  (9,'Фриерен','フリーレン','main','Эльфийская волшебница, пережившая своих спутников.','','1000+','Ж','Высший уровень магии');
